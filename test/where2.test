...
# Demonstrate that CROSS JOIN is a join reordering barrier.
#
reset_db
do_execsql_test where2-16.1 {
  CREATE TABLE t1(a INTEGER PRIMARY KEY, b INT);
...
# Here the query planner wants to move t4 forward so that it is in front of
# t1.  Ensure that does not happen.
do_execsql_test where2-16.2 {
  EXPLAIN QUERY PLAN
  SELECT *
...
# In this case the planner wants to move t1 to come after t4.  Ensure that
# does not happen.
do_execsql_test where2-16.2 {
  EXPLAIN QUERY PLAN
  SELECT *

...
# Want a Tcl version with 3-argument close (for an in/out pipe.)
...
proc decruft {text} {
  ...
  set nofnames [regsub -all {\w*(\.\w+)+} $noticket "" ]
  set nourls [regsub -all {\w+://} $nofnames "/" ]
  set nopaths [regsub -all {/\w+} $nourls "" ]
  set nounderv [regsub -all {[[:alnum:]]+(_[[:alnum:]]*)+} $nopaths "" ]
  return $nounderv
}
